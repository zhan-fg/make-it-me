...
export function buildPortraitPrompt(template: PortraitTemplate, mode: PortraitPromptMode) {
  ...
  const retouchRule = template.category === "id_photo"
    ? "只允许轻微磨皮和轻微均匀肤色，保留皮肤纹理、痣、面部轮廓与真实特征；不得美白过度、瘦脸、大眼或改变眼睛、鼻子、嘴唇和表情。"
    : "进行自然商业精修：均匀肤色、去除明显痘印黑眼圈和泛红、适度磨皮提亮、增加自然皮肤光泽、轻微收紧面颊与下颌；保留真实皮肤纹理，不大眼，不改变鼻子嘴唇，不强行露齿。";
