...
import { evaluateFrame, selectBestFrames } from "@/services/capture-quality";
...
export function GuidedCapture({ requirement, onComplete, onCancel }: Props) {
  ...
  const grabFrame = async (instructionId: string) => {
    ...
    return evaluateFrame(canvas, instructionId, boxes);
  };
  ...
  const runCapture = async () => {
    ...
    for (let index = 0; index < instructions.length; index += 1) {
      ...
      await wait(instructions[index].holdMs);
    ...
    if (!requirement.needsFullBody || fullBodyImage) {
      onComplete({ mode: requirement.mode, candidates, selectedFrames: selectBestFrames(candidates, requirement.bestFrameCount), fullBodyImage, detector: detector ? "browser-face-detector" : "local-quality-only" });
    }
  };
  ...
  return <div className="mx-auto max-w-[1080px] px-4 pb-12 pt-4 sm:px-6 sm:pt-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <section className="relative min-h-[570px] overflow-hidden rounded-[30px] bg-[#17171b] text-white shadow-xl">
        ...
        {cameraState === "ready" && <>
          <div className="absolute inset-x-0 top-0 bg-gradient-to-b from-black/65 to-transparent px-5 pb-20 pt-5"><p className="text-xs text-white/65">{requirement.mode === "simple" ? "自拍采集" : `${requirement.captureDurationSeconds} 秒引导采集`}</p><div className="mt-3 h-1 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-[#d8ff70] transition-all duration-500" style={{ width: `${running ? ((instructionIndex + 1) / instructions.length) * 100 : 0}%` }}/></div></div>
      ...
      <aside className="space-y-4">
        ...
        <details className="rounded-[20px] bg-white p-4 text-xs text-[#77777e]"><summary className="cursor-pointer text-[#333338]">检测说明</summary><p className="mt-3 leading-5">亮度和清晰度在浏览器本地真实计算。{detector ? "当前浏览器还提供了真实人脸数量与大小检测。" : "当前浏览器未提供 FaceDetector，因此头部方向提示是定时引导，不代表精确姿态检测；人脸、遮挡与姿态指标会标记为不可用。"}</p></details>
      </aside>
    </div>
  </div>;
}
