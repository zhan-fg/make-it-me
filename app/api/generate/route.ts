...
function refinementPrompt() {
  return ["这是生成完成后的身份保护 AI 美颜与环境融合步骤。只在框选人物区域内精修，不改变人物身份、脸型骨骼、五官比例、发型轮廓、身体比例、姿态、表情、嘴型、牙齿、服装、构图或背景内容。",
    "进行明显但仍自然的商业人像精修：充分均匀肤色，明显减弱痘印、黑眼圈、局部泛红、细纹和皮肤瑕疵，进行中高强度磨皮与提亮，同时保留少量毛孔和真实皮肤层次，避免塑料皮肤。",
    "进行适度瘦脸：主要收紧两侧面颊和下颌外轮廓，视觉缩窄约 8%–12%，保持原始颧骨、下巴长度、脸型识别特征和头部比例。禁止大眼、改变鼻子、改变嘴唇、重做牙齿、改变表情或把用户变成另一个人。",
