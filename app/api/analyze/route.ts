...
export async function POST(request: Request) {
  try {
    const configured = (process.env.ANALYZER_PROVIDER || "mock").toLowerCase();
    const provider: AnalyzerProvider = configured === "openai" || configured === "gemini" ? configured : "mock";
    const hasRemoteProvider = (provider === "openai" && Boolean(process.env.OPENAI_API_KEY)) || (provider === "gemini" && Boolean(process.env.GEMINI_API_KEY));
    ...
    if (provider === "mock") return NextResponse.json(mockAnalysis(configured === "mock" ? "ANALYZER_PROVIDER=mock" : `未知 provider: ${configured}`));

    const data = provider === "openai" ? await analyzeWithOpenAI(image) : await analyzeWithGemini(image);
