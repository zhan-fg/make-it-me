...
export default function Home() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/portrait-generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ templateId: template.id, selfieImageUrl: uploadPayload.url, beautyLevel }) });
  ...
  return <main className="min-h-screen bg-[#efede9] text-[#211f1d]">
    ...
    {step === "selfie" && template && <SelfiePage template={template} selfie={selfie} error={error} beautyLevel={beautyLevel} setBeautyLevel={setBeautyLevel} back={() => setStep("templates")} pick={() => input.current?.click()} generate={generate}/>}
