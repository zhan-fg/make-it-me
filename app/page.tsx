...
import type { CaptureResult, CharacterProfile, EphemeralIdentitySession, GenerationResult, IdentityProfile, IdentityRequirement, PhotoScenario, ReferenceAnalysis, TargetShot, TransferStrength } from "@/services/types";
...
function CaptureReview({ capture, requirement, retake, continueFlow }: { capture: CaptureResult; requirement: IdentityRequirement; retake: () => void; continueFlow: () => void }) {
  const matched = capture.selectedFrames.find((frame) => frame.targetPoseMatched) || capture.selectedFrames[0];
  return <Page><div className="mx-auto max-w-4xl"><span className="text-xs text-[#7655d6]">姿势匹配拍摄完成</span><h1 className="mt-2 text-[34px] font-bold">确认这张原始自拍</h1><p className="mt-2 text-sm text-[#757575]">采集阶段不进行美颜，保留真实五官与皮肤信息。生成完成后会统一执行身份保护 AI 精修。</p><div className="mx-auto mt-7 max-w-sm overflow-hidden rounded-[26px] bg-white"><div className="relative aspect-[3/4]"><img src={matched.dataUrl} alt="参考姿势匹配自拍" className="h-full w-full object-cover"/><span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1.5 text-[10px]">原始采集</span><span className="absolute bottom-3 right-3 rounded-full bg-[#d8ff70] px-3 py-1.5 text-[10px] font-medium">姿势匹配 {matched.quality.poseMatch.value ?? "—"}%</span></div></div>{capture.fullBodyImage && <div className="mx-auto mt-4 flex max-w-sm items-center gap-3 rounded-2xl bg-white p-3"><img src={capture.fullBodyImage} alt="全身参考" className="h-16 w-12 rounded-lg object-cover"/><div><b className="text-sm">全身参考已加入</b><p className="mt-1 text-xs text-[#77777e]">用于身体比例，不执行采集美颜</p></div><IconCheck size={18} className="ml-auto text-[#7655d6]"/></div>}<div className="mt-5 rounded-2xl bg-[#f4efff] p-4"><b className="text-sm">生成后自动 AI 精修</b><p className="mt-2 text-xs leading-5 text-[#6f6880]">只改善肤色、气色和轻微皮肤瑕疵，同时修复人物与环境的光线融合；不会主动瘦脸、大眼或改变表情和牙齿。</p></div><div className="mt-6 flex gap-3"><button onClick={retake} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-white py-4 text-sm"><IconRefresh size={17}/>重拍</button><button onClick={continueFlow} className="flex-[2] rounded-2xl bg-[#1d1d21] py-4 text-sm font-semibold text-white">生成并进行 AI 精修</button></div><p className="mt-4 text-center text-[11px] text-[#8b8b91]">{modeLabels[requirement.mode]} · 原始采集素材仅用于本次生成</p></div></Page>;
}
...
function Result({ reference, resultImage, generation, again, reset }: { reference?: Reference; resultImage?: string; generation?: GenerationResult; again: () => void; reset: () => void }) {
  return <Page><h1 className="text-[34px] font-bold">这张，是你的精修版本</h1><p className="mt-2 text-[15px] text-[#757575]">姿势匹配仿拍 · 身份保护 AI 美颜 · 环境融合</p><div className="mt-5 grid gap-4 lg:grid-cols-2"><div><ReferenceView reference={reference} className="h-[500px] rounded-[26px]"/><p className="mt-2 text-sm">参考素材</p></div><div><div className="h-[500px] overflow-hidden rounded-[26px] bg-gradient-to-br from-[#e5b99c] to-[#465272]">{resultImage ? <img src={resultImage} alt="生成并精修后的结果" className="h-full w-full object-cover"/> : null}</div><div className="mt-2 flex items-center gap-2 text-sm">我的版本 <Pill>AI 精修完成</Pill>{generation && <span className="text-xs text-[#77777e]">{generation.model} · {(generation.elapsedMs / 1000).toFixed(1)} 秒</span>}</div></div></div><div className="mt-6 flex flex-wrap items-center gap-2.5 text-[13px]"><a href={resultImage} download="make-it-me.png" className="flex items-center gap-1.5 rounded-full bg-white px-3.5 py-2"><IconDownload size={15}/>保存高清</a><button className="flex items-center gap-1.5 rounded-full bg-white px-3.5 py-2"><IconShare3 size={15}/>分享对比图</button><button onClick={again} className="rounded-full bg-[#1a1a1f] px-3.5 py-2 text-white">再拍一次</button><button onClick={reset} className="ml-auto text-[#757575]">完成并清空本次素材</button></div><div className="mt-5 flex items-start gap-3 rounded-2xl bg-[#f4efff] p-4 text-xs text-[#6f6880]"><IconLock size={16} className="shrink-0"/>展示的是完成 AI 美颜和环境融合后的最终结果。点击“完成并清空”会清除当前页面会话内的采集状态。</div></Page>;
}
